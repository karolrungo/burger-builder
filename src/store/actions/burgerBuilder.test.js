...
describe('burgerBuilder actions', () => {
  ...
  it('axios manual mock resolve', () => {
    const ingredients =  { data: { bacon: 1, meat: 0, salad: 1  } };
    axios.get = jest.fn();
    axios.get.mockImplementation( () => Promise.resolve(ingredients))

    const expectedActions = [
      { type: actionTypes.SET_INGREDIENTS, ingredients: ingredients.data },
    ];
    const store = mockStore({ posts: {} })
